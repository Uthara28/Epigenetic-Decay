...
nof_dimensions  = 1
only_gen        = 0 # 0-epi or 1-og in bash
...
def run_simulation(args):
    ...
    mean_phenotype = np.zeros(maxgen)
    std_phenotype= np.zeros(maxgen)
    ...
    for t in range(0, (maxgen-1)):
        ...
        std_phenotype[t] = np.std(N[:, dims['pheno']])
        mean_phenotype[t] = np.mean(N[:, dims['pheno']])
...
folder_path  = '/home/usriniva/Desktop/Phd/Epigenetics/Simulations/New_Hanna_eqns/Hanna_simu/results/new_results/phenvar'
...
if only_gen:
    og = 'onlygen'
    filename = f'rhos{rho_cases}_ms{m_cases}_gens{max_gen}_dims{nof_dimensions}_mutrate{mutrate[0]}_onlygen'

    
else:
    og = 'epi'
    filename = f'rhos{rho_cases}_ms{m_cases}_gens{max_gen}_dims{nof_dimensions}_mutrate{mutrate[0]}'
